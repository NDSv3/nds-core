#ifndef DEVICEFLOAT_H_
#define DEVICEFLOAT_H_
 ...
 *          of type double for testing purposes of nds-core V3.
 *
...
public:
     ...
     *  the device
     */
    static DeviceFloat* getInstance(const std::string& deviceName);
...
private:
    ...
    /**
     * @brief DeviceFloat state machine
     */
    nds::StateMachine m_DeviceFloat_stateMachine;

    /**
     * Methods to control DeviceFloat state machine
     */
    void switchOn_DeviceFloat();  ///< Called to switch on the DeviceFloat (rootnode).
    void switchOff_DeviceFloat(); ///< Called to switch off the DeviceFloat (rootnode).
    void start_DeviceFloat();     ///< Called to start the DeviceFloat (rootnode).
    void stop_DeviceFloat();      ///< Called to stop the DeviceFloat (rootnode).
    void recover_DeviceFloat();   ///< Called to recover the DeviceFloat (rootnode) from a failure.

    bool allow_DeviceFloat_Change(const nds::state_t, const nds::state_t, const nds::state_t); ///< Called to verify if a state change is allowed
     ...
     */
    nds::DataAcquisition<float> m_DataAcquisition;
     ...
     */
    nds::WaveformGeneration<float> m_WaveformGeneration;
